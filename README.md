...
**Updating from the server still works.** The sync never overwrites local work: with uncommitted changes or unpushed commits on `main` it stops and prints why (`journalctl --user -u pairwise-deploy`). To deploy what is checked out on the box, run `npm run deploy` (`scripts/deploy.sh --here`: test, build, restart, no fetch); once you `git push origin main`, the timer sees nothing new and stays quiet. `scripts/deploy.sh --force` rebuilds and restarts at the current commit after syncing. Watch a deploy with `journalctl --user -u pairwise-deploy -f`; `systemctl --user list-timers pairwise-deploy.timer` shows the next poll.

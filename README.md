...
Sessions live at `/session/<id>`: a refresh or back/forward rejoins the room with the current problem, editor, testcases, whiteboard strokes, transcript, run records, and revealed constraints (the server keeps a session for three idle hours). On the coding setup page, clicking rows in Matches pins those problems; Start uses the pinned problems in order and fills the remaining count at random. Multi-problem sessions have a problem picker in the topbar. In the coding room, `Ctrl/Cmd+'` runs the Testcase panel and `Ctrl/Cmd+Enter` submits; the bar above the Testcase/Test Result/Debugger tabs drags to resize the panel (double-click resets). History rows on the profile page expand to the full rubric.

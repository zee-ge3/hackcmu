...
Sessions live at `/session/<id>`: a refresh or back/forward rejoins the room with the current problem, editor, testcases, whiteboard strokes, transcript, run records, and revealed constraints (the server keeps a session for three idle hours). Sessions are held in memory and written through to SQLite (`interview_sessions`) shortly after every request, and restored at startup, so a deploy or restart does not end interviews in progress. On the coding setup page, clicking rows in Matches pins those problems; Start uses the pinned problems in order and fills the remaining count at random. Once an interview is graded it is read-only: reopening its URL goes to the profile, and a second Finish returns the stored feedback. For problems without a prepared suite the derived signature reads the statement: "in any order" switches the comparison to unordered (or triplets for nested arrays). Multi-problem sessions have a problem picker in the topbar. In the coding room, `Ctrl/Cmd+'` runs the Testcase panel and `Ctrl/Cmd+Enter` submits; the toolbar's reset control restores the starter code (two clicks); the probability room has a Hint button that asks the backend for the next smallest hint, shown as text under the question and spoken when voice is live (counts are passed to the grader); the bar above the Testcase/Test Result/Debugger tabs drags to resize the panel (double-click resets). The sun/moon control in the surface header switches the whole room (problem statement, editor, test panel, debugger, notes, whiteboard desk, transcript, top bar) between a green-tinted dark and a white theme; Monaco uses matching `pairwise-dark` / `pairwise-light` themes (`src/ide-theme.mjs`), the choice is remembered per browser, and the palette is a token set on `.workspace` (`--ide-*` for the work surface, `--pane-*` for the reading pane, `--d-*` for the chrome in `style.css`). History rows on the profile page expand to the full rubric.

## Interviewer behaviour

The voice interviewer reacts without being asked: a one-sentence spoken reaction after every Run or Submit (verdict, failing case, or a follow-up question), after each probability answer check, and when a design constraint is revealed. If the candidate says nothing for a minute, the reasoning backend is asked for a short check-in: one that acknowledges visible progress when they have been typing or drawing, or a gentle offer to think out loud when they have not. Follow-ups back off (two minutes, then four) while they stay quiet, and speaking resets the back-off; if the backend cannot answer, the voice agent is told to check in itself (`checkInDue` in `src/voice.mjs`). Saying "give me a minute", "let me think", or "stop talking" puts the interviewer on hold: the live agent is told to stay silent, spoken reactions and check-ins are suppressed, and the status pill reads _quiet_ until the candidate addresses it again (or ten minutes pass). The backend can also advance the interview with `next_problem` / `next_question` when the candidate asks to move on; the client switches problems and Alex introduces the next one. In coding rooms `add_testcases` lets the interviewer put cases into the candidate's Testcase panel (when asked, or to make a boundary or failing input concrete after the candidate has added their own); they are validated like the candidate's, marked with a sparkle in the panel, selected on arrival, and reported to the grader as `fromInterviewer` so they do not count as the candidate's coverage. Every mode also has `draw_on_whiteboard`: the backend sends boxes, circles, arrows, lines and labels on a 100×100 grid, the whiteboard tab opens, and the shapes are drawn one by one in Alex's own violet ink (then synced and described like any other change, without counting as candidate activity). `clear_whiteboard` removes Alex's own sketch (scope `mine`, before drawing a new one) or the whole board (scope `all`, only when the candidate asks).

**Prepared visuals (probability).** `data/probability/visuals/<id>.json` gives a question a setup diagram and a worked solution in steps, each with whiteboard shapes on the same 100×100 grid (`server/visuals.mjs` validates them at startup; `node scripts/check-visuals.mjs --render DIR` checks every file and rasterises the pictures with `scripts/render-sketch.py`). In the room, the **Diagram** button puts the setup picture on the whiteboard in its own ink; the backend's `show_visual` tool does the same, or draws the worked solution up to step k and returns that step's text so Alex can narrate it (steps count as hints for the grader: `solutionStepsShown`). After a solve or reveal the pane lists the worked steps with a **Show** button each. A visual may also supply the final `answer` for a question whose source has none (QuantProf), which makes grading exact. The probability setup page lets you pin up to five questions (◐ marks one with a visual); `data/probability/quantprof_extracted.json` holds questions extracted from QuantProf video transcripts and is merged into the bank at startup. Every mode also has `end_interview`: when the candidate asks to finish or wrap up (or the interview is complete), the backend's reply is spoken as the closing line, the client waits for it to be said, closes voice, and requests grading, the same as pressing Finish. When voice is not connected (no microphone, disconnected), backend replies are added to the transcript as text turns instead of being lost. In the probability and design rooms the backend has `read_notes` / `write_notes` for the candidate's scratch pad or design document (same revision check as the code editor); it only writes when asked, and the text is typed into the pad line by line like a code edit.
...
The production copy runs on one machine as the user service `pairwise.service` (built `dist/`, `NODE_ENV=production`, `PORT` from `.env`) behind a Cloudflare Tunnel; `scripts/deploy-setup.sh` adds the tunnel ingress (needs sudo).

**Continuous deployment from `main`.** `scripts/install-cd.sh` (no sudo) installs a user timer, `pairwise-deploy.timer`, that runs `scripts/deploy.sh` every minute, and generates `DEPLOY_WEBHOOK_SECRET` in `.env`. `deploy.sh` fetches `origin/main` and, only when it moved, fast-forwards, runs `npm ci` if `package-lock.json` changed, runs the unit tests, builds into `dist-next` and swaps it in, and restarts the service (live interviews survive the restart because sessions are persisted). For an instant deploy on push, add a GitHub webhook (repository Settings → Webhooks): payload URL `https://pairwise.georgez.xyz/hooks/github`, content type `application/json`, secret = `DEPLOY_WEBHOOK_SECRET`, event _push_. The public hostname sits behind a Cloudflare managed challenge, which blocks GitHub's delivery with a 403 until a WAF rule skips the challenge for the path `/hooks/github` (Cloudflare dashboard → Security → WAF → custom rule, action _Skip_); the timer works without it. The server verifies GitHub's `X-Hub-Signature-256` over the raw body and starts `pairwise-deploy.service`; pushes to other branches and pings are acknowledged without deploying. `.github/workflows/ci.yml` runs the unit tests and the build on every push and pull request.

**Updating from the server still works.** The sync deploys whatever `main` is checked out at when it differs from the last deployed commit, so a commit made and pushed from this box goes live within a minute like any other; unpushed local commits deploy too (the log says origin is behind until you push). It never overwrites local work: with uncommitted changes it stops and says so once (`journalctl --user -u pairwise-deploy`), and a divergence from `origin/main` is reported, not resolved. To deploy uncommitted edits, run `npm run deploy` (`scripts/deploy.sh --here`: test, build, restart, no fetch). `scripts/deploy.sh --force` rebuilds and restarts at the current commit. Watch a deploy with `journalctl --user -u pairwise-deploy -f`; `systemctl --user list-timers pairwise-deploy.timer` shows the next poll.
